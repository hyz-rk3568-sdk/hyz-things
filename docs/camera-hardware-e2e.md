...
时间戳水印在旋转、缩放之后、编码之前由 GStreamer `clockoverlay` 烧入码流：左上角、黑底、`%Y-%m-%d %H:%M:%S`，固定 `20px` 字号（`DejaVu Sans 20px`，所有预设一致）。水印是编码视频的一部分，不是浏览器叠加层；截图/录屏均包含它。验收截图应能在左上角看到与板端系统时间一致的日期时间文本。
