...
5. router uplink、PPPoE 凭据、OTA 和任意网络配置默认不通过 LAN API 暴露。
6. 若以后增加 uplink 配置，只允许受验证的枚举和 DTO，不接受 interface 或命令字符串。
...
- [ ] 构建并安装包含 S81 封顶指数退避修正的新固件。
- [ ] 重新执行自动冷启动和 management-only 可访问性验收。
