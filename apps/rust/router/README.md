...
- 提供只绑定管理 LAN 的 Axum API 和 Yew 状态/控制面板。
...
| 路径 | 用途 |
| --- | --- |
| `/userdata/hyz-router/wpa_supplicant.conf` | WAN STA 配置 |
| `/userdata/hyz-router/hostapd-sta-ap.conf` | AP 配置源 |
| `/userdata/hyz-router/mihomo/config.yaml` | Mihomo 配置源 |
...
Web 仅能调用固定、类型化的背光和代理控制接口；router enable/disable、OTA、任意命令、路径、URL 和原始 Mihomo controller 都不会暴露到 LAN API。写请求要求同源 `Origin`、JSON、启动期 CSRF token 和受限 DTO。该 token 用于 CSRF 防护，**不是用户认证**；管理 LAN 若允许不可信客户端，部署前必须增加独立认证与 HTTPS。
