...
test('renders the dashboard and applies anonymous typed controls', async ({ page, request }) => {
  ...
  await page.goto('/');
  await expect(page.getByRole('heading', { name: '网络状态', level: 1 })).toBeVisible();
  ...
  await expect(page.getByText('E2E-Upstream', { exact: true }).first()).toBeVisible();
...
test('supports the administrator, STA, AP, and write-only subscription journey', async ({
  ...
  await page.getByLabel('密码').fill('guest-password');
  await page.getByRole('button', { name: '检查并应用 STA' }).click();
  const staConfirmation = page.getByRole('region', { name: '应用上游 Wi-Fi？' });
