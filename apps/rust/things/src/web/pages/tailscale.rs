        ...
        (Some(snapshot), error) => {
            ...
            html! {
                <div class="grid gap-3 rounded-box border border-base-content/10 bg-base-200/40 p-4" role="region" aria-label="Tailnet 设备">
                    ...
                    <div class={BUTTON_ROW}><button class={BUTTON} type="button" onclick={refresh.clone()} disabled={state.tailscale_peers_meta.loading}>{"重新读取设备"}</button></div>
...
pub(crate) fn render_tailscale_control(state: &UseReducerHandle<AppState>, csrf: &str) -> Html {
    ...
    let Some(tailscale) = state.tailscale.as_ref() else {
        return html! {
            <SectionCard title_id="tailscale-title">
                ...
                <div class={BUTTON_ROW}><button class={BUTTON} type="button" onclick={retry_status} disabled={state.tailscale_meta.loading}>{"重试 Tailscale 状态"}</button></div>
            </SectionCard>
        };
    };
    ...
    html! {
        <SectionCard title_id="tailscale-title" busy={Some(busy)}>
            ...
            if let Some(error) = &state.tailscale_meta.error { <div class="grid gap-2"><ErrorState message={format!("Tailscale 状态刷新失败，保留最近数据：{error}")} /><div class={BUTTON_ROW}><button class={BUTTON} type="button" onclick={retry_status} disabled={state.tailscale_meta.loading}>{"重试状态"}</button></div></div> }
            ...
            <article class={INNER_CARD} role="region" aria-label="Tailscale 远程 LAN 状态">
                ...
                if needs_login {
                    ...
                    if let Some(login_url) = &state.tailscale_login_url {
                        <div class={BUTTON_ROW}><a class={BUTTON_PRIMARY} href={login_url.clone()} target="_blank" rel="noopener noreferrer">{"打开一次性 Tailscale 登录链接"}</a><button class={BUTTON} type="button" onclick={enable.clone()} disabled={busy}>{"已完成登录，继续启用"}</button></div>
                    } else {
                ...
                <div class={BUTTON_ROW} role="group" aria-label="Tailscale 操作">
                    <button class={if lan_access_ready { BUTTON } else { BUTTON_PRIMARY }} type="button" onclick={enable} disabled={busy || lan_access_ready} aria-pressed={lan_access_ready.to_string()}>{enable_label}</button>
                    <button class={BUTTON} type="button" onclick={disable} disabled={busy || disabled_ready} aria-pressed={disabled_ready.to_string()}>{disable_label}</button>
